...
st.set_page_config(page_title='Visão País', page_icon='🌎', layout='wide') #'wide' = use full screen
...
tab1, tab2,tab3  = st.tabs( [ 'Visão Geral', 'Visão Estratégica', 'Visão Descritiva'])
...
with tab1:
    with st.container():
        ...
        fig = px.bar((df_aux.head(10)),x='country_name',y='restaurant_id',template='plotly_dark', text_auto=True )

        st.markdown('#### Top 10 Países com mais restaurantes cadastrados')
        st.plotly_chart(fig, use_container_width=True)

        col1, col2 = st.columns(2,gap='large')
        with col1:
            df_aux = group_country(df,'city','nunique')
            fig = px.bar((df_aux.head(10)),x='country_name',y='city',template='plotly_dark',  text_auto=True)

            st.markdown('#### Top 10 países com mais cidades cadastradas')
            col1.plotly_chart(fig, use_container_width=True)

        with col2:
            df_aux = (df.loc[df['price_range'] == 'gourmet',['country_name','price_range']].groupby(['country_name'])
                                                                                .count()
                                                                                .sort_values(by='price_range', ascending=False)
                                                                                .reset_index())

            #gráfico países com mais restaurantes gourmet
            fig = px.pie((df_aux.head(10)),names='country_name',values='price_range',template='plotly_dark')

            fig.update_traces(textposition='inside', textinfo='percent+label')

            st.markdown('#### Países com restaurantes Gourmet')
            col2.plotly_chart(fig, use_container_width=True)

with tab2:
    with st.container():
        df_aux = (df.loc[:,['country_name','average_cost_for_two','currency']].groupby(['country_name','currency'])
                                                    .mean()
        ...
        fig = px.bar((df_aux.head(10).sort_values(by='average_cost_for_two',
                                                    ascending=True)), 
        ...
        st.markdown('#### Média da quantidade de avaliações feitas po País')
        st.plotly_chart(fig, use_container_width=True)

        col1, col2 = st.columns(2,gap='large')
        with col1:

            df_aux = group_country(df,'votes','mean')
            #Gráfico Média de avaliações feitas por país
            fig = px.bar((df_aux.head(10)),x='country_name',y='votes',template='plotly_dark',text_auto=True )

            st.markdown('#### Média da quantidade de avaliações feitas po País')
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            df_aux = group_country(df,'aggregate_rating','mean')
            fig = px.bar(df_aux,x='country_name',y='aggregate_rating',template='plotly_dark', text_auto=True )

            st.markdown('#### Média da quantidade de avaliações feitas po País')
            st.plotly_chart(fig, use_container_width=True)
with tab3:
    with st.container():
        colunas = ['country_name','currency','votes','average_cost_for_two','aggregate_rating','cuisines','restaurant_name']
        df_aux = (df.loc[:,colunas].groupby(['country_name','currency'])
